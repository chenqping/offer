 ...
 *    算法说明：先要考虑大整数的表示方式，使用long,BigInteger不符要求，也会超出范围，考虑使用数组来表示，即从高
 *    依次存入数组中。通过分析两数相乘的计算过程，可知大致可以采用[A,B]*[C,D]=[AC,(AD+BC),BD]先计算出中间结果，
 *    然后对结果进位处理，积的个位为BD%10，个位会向十位产生进位，进位的数值是BD/10，积的十位要加上个位的进位，所以
 *    结果就是(AD+BC+进位)%10，同样十位也会产生向百位的进位，依此类推得出进位处理后的就是积，对于积要去掉前导的0值
 *    再转换为字符串。
 *    边界条件：特殊用例，乘数为0的情况
 *    考察点:
